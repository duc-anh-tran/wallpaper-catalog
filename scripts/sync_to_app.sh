...
echo "=== Syncing bundled videos to Android app ==="
...
while IFS= read -r filename; do
    ...
    if [ ! -f "$src" ]; then
        echo "  WARNING: Missing video: $filename (referenced in wallpapers.json but not in originals/videos/)"
        ((missing++))
        continue
    fi
...
echo ""
echo "=== Sync complete ==="
echo "  Videos copied: $copied"
echo "  Missing videos: $missing"
